...
export default function AlumniPage() {
  ...
  const [alumniYearData, setAlumniYearData] = useState<AlumniYearStatsDocument[]>([])
  const [highlightedAlumni, setHighlightedAlumni] = useState<FeaturedAlumniDocument[]>([])
  const [alumniStories, setAlumniStories] = useState<AlumniStoryDocument[]>([])
  ...
  const fetchData = async () => {
    try {
      ...
      const [statsRes, alumniRes, storiesRes] = await Promise.all([
        fetch('/api/alumni/stats'),
        fetch('/api/alumni/featured?featured=true&limit=6'),
        fetch('/api/alumni/stories?published=true&limit=3')
      ])
      ...
      setAlumniYearData(statsData.stats || [])
      setHighlightedAlumni(alumniData.alumni || [])
      setAlumniStories(storiesData.stories || [])
    } catch (error) {
      console.error('Error fetching alumni data:', error)
      // Set empty arrays on error
      setAlumniYearData([])
      setHighlightedAlumni([])
      setAlumniStories([])
    } finally {
      setLoading(false)
    }
    ...
    <main className='min-h-screen bg-gradient-to-br from-primary-50/50 via-white to-primary-green-50/50'>
      ...
      <Section background='gray'>
        <div className='max-w-7xl mx-auto'>
          ...
          {/* Yearwise Statistics */}
          {alumniYearData.length > 0 && (
            <div className='mb-16'>
              <h2 className='text-3xl md:text-4xl font-bold text-gray-900 mb-8 text-center'>
                Alumni by Graduation Year
              </h2>
              <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6'>
                {alumniYearData.map((item) => (
                  <Card
          ...
          {/* Empty States */}
          {!loading && highlightedAlumni.length === 0 && alumniStories.length === 0 && (
            <div className='text-center py-12'>
              <p className='text-gray-600 text-lg'>No alumni content available at this time.</p>
            </div>
          )}
        </div>
      </Section>
